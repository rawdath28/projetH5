...
export async function analyzeTextWithMistral(text: string): Promise<AnalysisResult> {
  ...
  // Debug: vérifier si la clé est bien récupérée (masquer les caractères sauf les 4 premiers)
  if (apiKey) {
    const maskedKey = apiKey.substring(0, 4) + '...' + apiKey.substring(apiKey.length - 4);
    console.log('Clé API récupérée:', maskedKey);
  } else {
    console.error('MISTRAL_API_KEY non trouvée dans les variables d\'environnement');
